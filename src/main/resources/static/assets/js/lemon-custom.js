...
;(function($, window, undefined){
	...
	{		
		...
		public_vars.$fixedTable           = public_vars.$mainContent.find('.fixed-table-body');
	...
	$(document).ajaxSend(function(){
		 show_loading_bar({delay : .3, pct : 100});
	}).ajaxComplete(function(){
		 hide_loading_bar();
	}).ajaxSuccess(function(){
	     //alert("请求成功后");
	}).ajaxError(function(event, xhr, options, errorMsg){
		if (xhr.status===401) {
			//options.url
...
function sidebar_menu_item_collapse($li, $sub)
{
	...
	TweenMax.to($sub, sm_duration, {css: {height: 0}, onUpdate: ps_update, onComplete: function()
	{
		...
		$li.find('li.expanded ul').attr('style', '').parent().removeClass('expanded');
...
function stickFooterToBottom()
{
	...
	if(public_vars.$mainFooter.hasClass('sticky'))
	{
		var win_height				 = jQuery(window).height(),
		    ...
		    fixed_table_height       = public_vars.$fixedTable.outerHeight();
		
		if (fixed_table_height && fixed_table_height<350) {
